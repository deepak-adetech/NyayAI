...
function emailWrapper(content: string, previewText?: string): string {
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>NyayAI</title></head>
<body style="margin:0;padding:0;background:#f8fafc;">
  ...
  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:560px;margin:0 auto;background:#f8fafc;padding:20px;">
    <div style="text-align:center;padding:20px 0 12px;">
      <span style="font-size:24px;font-weight:700;color:#1e3a5f;">NyayAI</span>
      <p style="margin:4px 0 0;font-size:12px;color:#64748b;text-transform:uppercase;letter-spacing:1px;">AI-Powered Legal Intelligence</p>
    </div>
    ...
    <div style="text-align:center;color:#94a3b8;font-size:12px;margin-top:24px;">
      <p style="margin:0 0 4px;">NyayAI &middot; AI-Powered Legal Intelligence</p>
      <p style="margin:0 0 4px;">Questions? <a href="mailto:support@nyayasahayak.com" style="color:#1e3a5f;">support@nyayasahayak.com</a></p>
      <p style="margin:8px 0 0;">&copy; 2025 NyayAI. All rights reserved.</p>
    </div>
  </div>
</body>
</html>`;
...
export function trialEndingEmail(name: string, daysLeft: number, upgradeUrl: string, discountCode?: string): string {
  ...
  return emailWrapper(content, `Your NyayAI trial ends in ${daysLeft} day${daysLeft === 1 ? "" : "s"}. Upgrade now to keep access.`);
}
...
export function subscriptionExpiredEmail(name: string, reactivateUrl: string, discountCode: string): string {
  const content = `
    ...
    <h2 style="margin:0 0 8px;font-size:22px;color:#1e293b;">Your access to NyayAI has paused</h2>
  ...
  return emailWrapper(content, `Your NyayAI trial ended. Use code ${discountCode} for 20% off when you reactivate.`);
}
...
export function paymentFailedEmail(name: string, updateUrl: string): string {
  ...
  return emailWrapper(content, "Action required: We could not process your NyayAI subscription payment.");
}
...
export function paymentSuccessEmail(name: string, plan: string, amount: string, nextBillingDate: Date | string): string {
  ...
  return emailWrapper(content, `Payment of ${amount} received. Your NyayAI ${plan} subscription is active.`);
}
...
export function subscriptionRenewingEmail(name: string, plan: string, amount: string, nextBillingDate: Date | string): string {
  ...
  const content = `
    ...
    <p style="color:#475569;margin:0 0 20px;line-height:1.6;">Hi ${name}, your NyayAI subscription renews automatically in 3 days.</p>
  ...
  return emailWrapper(content, `Reminder: Your NyayAI ${plan} plan renews on ${nextDate} for ${amount}.`);
}
...
export function retentionOfferEmail(name: string, discountCode: string, discountPercent: number, expiryDate: Date | string): string {
  ...
  const content = `
    ...
    <p style="color:#475569;margin:0 0 20px;line-height:1.6;">We have reserved a special offer exclusively for you to rejoin NyayAI.</p>
  ...
  return emailWrapper(content, `${name}, your exclusive ${discountPercent}% off offer for NyayAI expires on ${expiry}.`);
}
